...
{
    wallTexture = "General/Wall"; // wall texture to clean up the look of the senior game. The wall panels will be applied this texture rather than the texture set for the 'ground'
    moveLeft = false;
    moveRight = false;
    ...
    signalTypes[POD_SIGNAL_1].push_back(PodInfo(POD_SIGNAL_1, POD_FLOWER, POD_COLOR_BLUE, POD_SHAPE_ROSE, POD_SOUND_1));
    ...
    navMap = std::vector<NavigationLevel>(7);
...
void Util::copySides(bool dest[NUM_DIRECTIONS], bool src[NUM_DIRECTIONS])
{
...
void Util::createSegment(Ogre::SceneManager* sceneMgr, const std::string& strName, float length, float depth, const bool sides[NUM_DIRECTIONS])
{
    ...
    /*
    if( sides[WEST] ) { // 5 Lane Walls ---------------
        // WEST WALL
        move = Vector3(-wallLength - wallLength/2 - wallLength*Ogre::Math::Cos(Ogre::Degree(30)), -length/2 + wallLength/2 + wallLength*Ogre::Math::Sin(Ogre::Degree(30)), 0);
        ...
        // WEST WALL
        move = Vector3(-wallLength - wallLength/2, -length/2 + wallLength/2, 0);
        rot.FromAngleAxis(Degree(270), Vector3(0, 0, 1));
        createSubPlane(sceneMgr, manual, wallLength, depth, move, rot, bl, tr);
    ...
    */
    ...
    if (sides[EAST])
    {
        move = Vector3(wallLength*2 - 0.5 * wallLength * Ogre::Math::Sin(Degree(30)) * Ogre::Math::Cos(Degree(75)) / Ogre::Math::Sin(Degree(75)), -length / 2 + wallLength * Ogre::Math::Sin(Degree(30)) / 2, 0);
        rot.FromAngleAxis(Degree(30), Vector3(0, 0, 1));
        createSubPlane(sceneMgr, manual, wallLength, depth, move, rot, bl, tr);
    }
    
    if (sides[SOUTHEAST])
    {
        move = Vector3(wallLength, -length / 2, 0);
        rot.FromAngleAxis(Degree(0), Vector3(0, 0, 1));
        createSubPlane(sceneMgr, manual, wallLength, depth, move, rot, bl, tr);
    }
