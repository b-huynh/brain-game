...
{
    materialNames.clear();
    materialNames.push_back("General/Grass");
}
...
SectionInfo Tunnel::getNextSectionInfo() const
{
    ...
    Direction segmentDir = Util::randDirection(sides);
    int segmentTurnAngle = 2;//Util::randRangeInt(0, 2);//4);
...
PodInfo Tunnel::getNextPodInfoAt(SectionInfo segmentInfo, SetPodTarget setting)
{
    ...
    if (getMode() == GAME_NAVIGATION || getMode() == GAME_TEACHING || getMode() == GAME_RECESS)
    {
        ...
        ret.meshType = POD_FLOWER;
        ret.podColor = POD_COLOR_UNKNOWN;
...
void Tunnel::constructTunnel(int size)
{
