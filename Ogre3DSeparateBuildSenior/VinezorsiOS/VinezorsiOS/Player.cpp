...
PlayerLevel::PlayerLevel() : sessionID(0), set1Rep(0), set2Rep(0), set3Rep(0), set1(1), set2(1), set3(1), set1Notify(0), set2Notify(0), set3Notify(0), navigation(2), minSpeed(15), averageSpeed(20), maxSpeed(25), runSpeed1(15), runSpeed2(15), runSpeed3(15), navigationScores(), speedScores()
{}
...
void Player::testPodGiveFeedback(Pod* test)
{
    ...
    else if (!test->isPodGood() && !test->isPodTaken())
    {
        ...
        test->assertRetractAnimationFlag();
...
bool Player::checkPerformLeftMove(bool force)
{
    ...
    {
        //globals.moveLeft = true; -- Moved to activatePerformLeftMove() in OgreApp.cpp because IOS controls don't use this function
        return true;
    }
    else return false;
}
...
bool Player::checkPerformRightMove(bool force)
{
    if (force || (!inputMoved && ((globals.swipeInverted && inputTotalX < inputLeftBound) || (!globals.swipeInverted && inputTotalX > inputRightBound))))
    {
        //globals.moveRight = true;  -- Moved to activatePerformRightMove() in OgreApp.cpp because IOS controls don't use this function
        return true;
    }
    else return false;
}
...
void Player::update(float elapsed)
{
    ...
    for (int i = 0; i < vines.size(); ++i)
    {
        ...
        if (closest) {
            ...
            {
                targetPos1 = centerPos + closest->requestWallDirection(vines[i]->loc) * closest->getWallLength() / posOffset;
                targetPos2 = centerPos + closest->requestWallDirection(vines[i]->dest) * closest->getWallLength() / posOffset;
            }
            else
            {
                ...
                Vector3 p1 = exit + lookback->requestWallDirection(vines[i]->loc) * lookback->getWallLength() / posOffset;
                Vector3 p2 = entry + closest->requestWallDirection(vines[i]->loc) * closest->getWallLength() / posOffset;
                Vector3 p3 = exit + lookback->requestWallDirection(vines[i]->dest) * lookback->getWallLength() / posOffset;
                Vector3 p4 = entry + closest->requestWallDirection(vines[i]->dest) * closest->getWallLength() / posOffset;
