...
void Tunnel::update(Player* player, Hud* hud, float elapsed)
{
    ...
    if (!isDone())
    {
        ...
        else if (getMode() == GAME_TIMED)
        {
            ...
            {
                if (getPodIndex() >= globals.stageTimeThreshold2 + nback)
                    setDone(PASS);
                else
                    setDone(EVEN);
            }
